...
impl fmt::Display for IngredientParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            IngredientParseError::EmptyInput => write!(f, "Input text is empty or contains only whitespace"),
            IngredientParseError::NoIngredientFound => write!(f, "Could not identify any ingredient name"),
            ...
            IngredientParseError::AmbiguousParsing(options) => {
                write!(f, "Ambiguous parsing - multiple interpretations: {}", options.join(", "))
            }
...
impl Quantity {
    ...
    pub fn to_decimal(&self) -> Option<f64> {
        match self {
            ...
            Quantity::Fraction { whole, numerator, denominator } => {
...
fn extract_quantity_and_unit<'a>(words: &'a [&'a str]) -> Result<(Option<Quantity>, Option<Unit>, Vec<&'a str>), IngredientParseError> {
    if words.is_empty() {
...
fn try_parse_quantity(word: &str) -> Option<Quantity> {
    ...
    if word.contains('-') && word.chars().nth(0) != Some('-') {
        let parts: Vec<&str> = word.split('-').collect();
        if parts.len() == 2 {
...
mod tests {
    ...
    fn test_parse_fraction_ingredient() {
        ...
        if let Some(Quantity::Fraction { whole, numerator, denominator }) = result.quantity {
    ...
    fn test_parse_textual_quantity() {
        ...
        assert_eq!(result.quantity, Some(Quantity::Textual("pinch".to_string())));
        assert_eq!(result.unit, None);
